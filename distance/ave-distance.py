...
def get_alpha_way_AVD(df_old,df_new,alpha):
    n = df_old.shape[0]
    alpha_way_set = get_subset(df_old.keys(),alpha)
    number_of_alphaway = len(alpha_way_set)
    total_variation_distance = 0.0
    if alpha ==2:
        for alpha_way in alpha_way_set:
            alpha_old_pr = np.array([])
            alpha_new_pr = np.array([])
            atr1 = alpha_way[0]
            atr2 = alpha_way[1]
            domain_atr1 = set(df_old[atr1].unique())
            domain_atr2 = set(df_old[atr2].unique())
            for v1 in domain_atr1:
                for v2 in domain_atr2:
                    alpha_old_pr = np.append(alpha_old_pr,df_old[(df_old[atr1]==v1) & (df_old[atr2]== v2)].shape[0]/n)
                    alpha_new_pr = np.append(alpha_new_pr,df_new[(df_new[atr1]== v1) & (df_new[atr2]== v2)].shape[0]/n )
            total_variation_distance += np.sum((1/2)*np.abs(alpha_old_pr-alpha_new_pr))
    if alpha ==3:
        for alpha_way in alpha_way_set:
            alpha_old_pr = np.array([])
            alpha_new_pr = np.array([])
            ...
            domain_atr1 = set(df_old[atr1].unique())
            domain_atr2 = set(df_old[atr2].unique())
            domain_atr3 = set(df_old[atr3].unique())
            for v1 in domain_atr1:
                for v2 in domain_atr2:
                    for v3 in domain_atr3:
                        alpha_old_pr = np.append(alpha_old_pr,df_old[(df_old[atr1]==v1)&(df_old[atr2]== v2)&(df_old[atr3]== v3)].shape[0]/n)
                        alpha_new_pr = np.append(alpha_new_pr,df_new[(df_new[atr1]== v1)&(df_new[atr2]== v2)&(df_new[atr3]== v3)].shape[0]/n )
            total_variation_distance += np.sum((1/2)*np.abs(alpha_old_pr-alpha_new_pr))
    return total_variation_distance/number_of_alphaway
...
pridict_variable = {}
pridict_variable['Adult'] = ['income']  # ,]education'
# pridict_variable['NLTCS'] = [' eating']  # ,'bathing']
# pridict_variable['BR2000'] = ['religion']  # religion, 'car',
expriment_table = pd.DataFrame(index=epsilon_list)
avralpha = 0
frac = 1
alpha = 2
theta_usefulness = 4
...
for i in range(0,1):
    df_o = datasets[i]
    df, df_test = dataset_split_save(df_o, frac)
    score_SA_PrivBayes_list = []
    score_PrivBayes_list = []
    score_EL_PrivBayes_list = []
    MIM = get_mim(df)
    d = df.shape[1]
    n = df.shape[0]
    if i == 0:
    ...
    for epsilon in epsilon_list:
        ...
        for _ in range(3):
           # **********************************************
            ...
            N, score = PrivBayes.PrivBayes_PDconstruct_Bayes(df, k=k, first_node=random_node, matrix=MIM,
                                                             epsilon=epsilon1, sensitivity=sensitivity)
            P = PrivBayes.NoisyConditionals(N, df, k, epsilon2)
            df_new = PrivBayes.sampling(N, df, P)
            avealpha=get_alpha_way_AVD(df, df_new, alpha)
            score_sum += avealpha
            end_new = time.time()
        ...
        for _ in range(3):
            ...
            N, score = PrivBayes_PDconstruct_Bayes(df, k=k, first_node=our_node, matrix=MIM,
                                                   epsilon=epsilon1, sensitivity=sensitivity)
            P = PrivBayes.NoisyConditionals(N, df, k, epsilon2)
            df_new = PrivBayes.sampling(N, df, P)
            avealpha = get_alpha_way_AVD(df, df_new, alpha)
        ...
        for _ in range(3):
            ...
            N, score, score_all = PrivBayes_PDconstruct_Bayes_new1(df, k=k, first_node=our_node, matrix=MIM,
                                                   epsilon=epsilon1, sensitivity=sensitivity)
            P = PrivBayes.NoisyConditionals_new(N, df, k, epsilon2, 1.1, score_all)
            df_new = PrivBayes.sampling(N, df, P)
            avealpha = get_alpha_way_AVD(df, df_new, alpha)
            score_sum += avealpha
            end_new = time.time()
